<!-- This is the view file for displaying a list of people. -->
...
<html>
    ...
    <body>
        <!-- The header section containing the title and menu -->
        <div class="header">
            <h1>People List</h1>
            @include('menu')
        </div>
        ...
        <table>
            ...
            @foreach($person as $p)
            <tr>
                <!-- Each row contains a link to view a person's details -->
                <td><a href="{{ route('person.show', $p->id) }}">{{ $p->first_name }}</a></td>
                <td>{{ $p->last_name }}</td>
                <td>{{ $p->email }}</td>
                <td>{{ date('M j, Y', strtotime($p->date_of_birth )) }}</td>
                <td>
                    <!-- Each row contains an edit button that redirects to the person edit form -->
                    <a href='{{ route('person.edit', $p->id) }}' class="edit button">Edit</a>
                    <!-- Each row contains a delete button that allows deleting a person -->
